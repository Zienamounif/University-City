...
class AuthController extends Controller
{
    ...
    public function signUp(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'governorate' => ['string', 'max:50'], //, 'in:latakia,tartous,aleppo,damascus,daraa,deir_ezor,hama,homs,idlib,quneitra,raqqa,rif_dimashq,hasakah,swedaa'
            'email' => ['required', 'string', 'max:100', 'email', 'unique:students', 'unique:admins'],
            'phone' => ['string', 'unique:students'],
            'password' => ['required', 'string', 'min:8'],
            'is_disabled' => ['boolean'],
            'collage' => ['string'],
            'collage_id' => ['string', 'unique:students'],
            'year' => ['string'], //, 'in:first,second,third,forth,fifth,sixth'
            'is_successded' => ['boolean'],
        ]);
